...
def get_sentence_embeddings(num):
    if num < 3:
        randomx = random.randint(100, 149)
    elif num == 3:
        randomx = random.randint(0, 49)
    else:
        randomx = random.randint(50, 99)

    embedding_model = SentenceTransformer('./embedding/get_embeddings')
    with open('sentence_embeddings.txt', 'r') as file:
        sentences = file.readlines()
        sentence = sentences[randomx]
    sentence_embeddings = embedding_model.encode(sentence, convert_to_tensor=True)
...
class PromptTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            embedding = get_sentence_embeddings(de_id)
            if de_id == 0:
        ...
        return [clean_name, de_id], degrad_patch, clean_patch, embedding
...
class DenoiseTestDataset(Dataset):
    ...
    def __getitem__(self, clean_id):
        embedding = get_sentence_embeddings(2)
        ...
        return [clean_name], noisy_img, clean_img, embedding
...
class DerainDataset(Dataset):
    ...
    def _add_gaussian_noise(self, clean_patch):
        noise = np.random.randn(*clean_patch.shape)
        noisy_patch = np.clip(clean_patch + noise * 25, 0, 255).astype(np.uint8)
        return noisy_patch, clean_patch
    ...
    def __getitem__(self, idx):
        embedding = get_sentence_embeddings(3)
        ...
        # degraded_img, _ = self._add_gaussian_noise(degraded_img)
        # clean_img, _ = self._add_gaussian_noise(clean_img)
        clean_img, degraded_img = self.toTensor(clean_img), self.toTensor(degraded_img)
        degraded_name = degraded_path.split('/')[-1][:-4]

        return [degraded_name], degraded_img, clean_img, embedding
...
class DehazeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        embedding = get_sentence_embeddings(4)
        degraded_path = self.ids[idx]
        clean_path = self._get_gt_path(degraded_path)
