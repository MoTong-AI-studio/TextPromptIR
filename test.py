...
from utils.dataset_utils import DenoiseTestDataset, DerainDataset, DehazeDataset
...
def test_Denoise(net, dataset, sigma=15):
    ...
    with torch.no_grad():
        for ([clean_name], degrad_patch, clean_patch, embedding) in tqdm(testloader):
            degrad_patch, clean_patch, embedding = degrad_patch.cuda(), clean_patch.cuda(), embedding.cuda()

            restored = net(degrad_patch, embedding)
            temp_psnr, temp_ssim, N = compute_psnr_ssim(restored, clean_patch)
            psnr.update(temp_psnr, N)
            ssim.update(temp_ssim, N)
            save_image_tensor(restored, output_path + clean_name[0] + '.png')
        print("Denoise sigma=%d: psnr: %.8f, ssim: %.8f" % (sigma, psnr.avg, ssim.avg))
...
def test_Derain_Dehaze(net, dataset, task="derain"):
    ...
    with torch.no_grad():
        for ([degraded_name], degrad_patch, clean_patch, embedding) in tqdm(testloader):
            degrad_patch, clean_patch, embedding = degrad_patch.cuda(), clean_patch.cuda(), embedding.cuda()
        ...
        print("PSNR: %.8f, SSIM: %.8f" % (psnr.avg, ssim.avg))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dehaze_path', type=str, default="test/dehaze/", help='save path of test input images')
    parser.add_argument('--output_path', type=str, default="output/", help='output save path')
    parser.add_argument('--ckpt_name', type=str, default="epoch=62-step=1663515.ckpt", help='checkpoint save path')
    ...
    if testopt.mode == 1:
        ...
        testopt.dehaze_path = dehaze_base_path
        dehaze_set = DehazeDataset(testopt, task="dehaze", addnoise=False, sigma=15)
